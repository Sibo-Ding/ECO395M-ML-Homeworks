...
To analyze the delay for all Austin departure flights, I categorize the departure time into 24 hours. The delay is shorter in the morning (5:00-9:00) and relatively steady afterwards. The gets longer at night (after 21:00).  
The delay is quite even for every day in a week and every month in a year.

```{r}
abia <- read.csv("ABIA.csv")

abia <- abia |> mutate(DepHour = floor(DepTime / 100)) |> mutate(DepHour = factor(DepHour))
...
```{r}
avg_swim_age <- top20 |> filter(sport == "Swimming") |> group_by(year, sex) |> summarize(avg_age = mean(age))
head(avg_swim_age)
```
